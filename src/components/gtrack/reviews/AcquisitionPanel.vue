      ...
      <template #content>
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="col-span-12 lg:col-span-5">
                ...
                <div v-else class="flex flex-col gap-6 h-full overflow-y-auto pr-1 custom-scrollbar font-sans" style="max-height: 700px;">
...
onMounted(async () => {
    ...
    try {
        ...
        // Merge defaults if new fields are missing from mock
        settings.value = { 
            ...settings.value, 
            ...s,
        ...
        if (settings.value.theme_color.startsWith('#')) settings.value.theme_color = settings.value.theme_color.substring(1);
        if (settings.value.qr_color && settings.value.qr_color.startsWith('#')) settings.value.qr_color = settings.value.qr_color.substring(1);
        if (settings.value.badge_color && settings.value.badge_color.startsWith('#')) settings.value.badge_color = settings.value.badge_color.substring(1);
        if (settings.value.qr_border_color && settings.value.qr_border_color.startsWith('#')) settings.value.qr_border_color = settings.value.qr_border_color.substring(1);
...
const saveSettings = async () => {
    ...
    try {
        ...
        toast.add({ severity: 'success', summary: 'Zapisano', detail: 'Ustawienia pozyskiwania zaktualizowane', life: 3000 });
    } catch (e) {
...
const downloadFile = (type, format) => {
    ...
    if (format === 'a4') {
        dimensions = '210mm x 297mm (2480x3508px @ 300DPI)';
    } else if (format === 'card') {
        dimensions = '54mm x 94mm (638x1110px @ 300DPI)';
    } else {
        dimensions = 'High Res';
    }
