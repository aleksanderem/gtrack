<template>
    <div>
        <div class="block-category-title">Detail</div>
        <BlockViewer header="Applicant Detail" :code="block1">
        ...
        <BlockViewer header="Listings" :code="block2" recent>
            <section class="surface-ground">
                <nav class="surface-overlay px-6 lg:px-3 xl:px-6 border-bottom-1 surface-border shadow-2 flex justify-content-between relative lg:static" style="min-height: 84px">
                    ...
                    <a
                        v-ripple
                        class="p-ripple cursor-pointer block lg:hidden align-self-center text-700"
                        v-styleclass="{
                            selector: '@next',
                            enterClass: 'hidden',
                            leaveToClass: 'hidden',
                    ...
                    <div class="flex-grow-1 justify-content-between hidden lg:flex absolute lg:static w-full surface-overlay left-0 top-100 z-1 shadow-2 lg:shadow-none">
                        <ul class="list-none p-0 m-0 flex select-none flex-column lg:flex-row">
                            <li>
                                <a
                                    v-ripple
                                    class="p-ripple flex px-6 p-3 h-full lg:px-3 lg:py-2 align-items-center text-600 border-left-2 lg:border-bottom-2 lg:border-left-none border-transparent hover:border-blue-500 font-medium cursor-pointer transition-colors transition-duration-150"
                                >
                                    <i class="pi pi-home mr-2"></i>
                                    <span>Home</span>
                                </a>
                ...
                <section class="surface-section px-6 py-5">
                    <header class="flex w-full justify-content-between align-items-center border-bottom-1 surface-border pb-5">
                        ...
                        <Button v-ripple class="p-ripple p-button-outlined" label="Report Issue"></Button>
                    </header>
                    <section class="flex flex-wrap flex-column md:flex-row md:align-items-center gap-2 py-5">
                        <MultiSelect :options="cities" v-model="selectedCities" class="w-full md:w-15rem" placeholder="Select a city" optionLabel="name" selectedItemsLabel="{0} items selected"></MultiSelect>
                        <Calendar v-model="rangeDates" selectionMode="range" class="w-full md:w-15rem" readonlyInput inputId="range"></Calendar>
                        <Dropdown :options="prices" v-model="selectedPrice" class="w-full md:w-15rem" placeholder="Select price range" optionLabel="value"></Dropdown>
                    </section>
                    <img src="/images/blocks/maps/map-1.png" alt="Map" class="w-full border-1 surface-border border-round" />
                    <div class="flex w-full justify-content-center sm:justify-content-end align-items-center my-5">
                        <SelectButton :options="stateOptions" v-model="value1" optionLabel="label" optionValue="value"></SelectButton>
                    </div>
                    <DataView
                        ref="dv"
                        :value="products"
                    ...
                    >
                        <template #list="{ data }">
                            <article class="flex flex-column md:flex-row w-full gap-3 p-3 border-round border-1 surface-border mb-3">
                                <div class="relative">
                                    <img :src="data.image" alt="Image" class="border-round w-full h-full md:w-14rem md:h-14rem" />
                                ...
                                <div class="flex flex-column w-full gap-3">
                                    <div class="flex w-full justify-content-between align-items-center">
                                        ...
                                        <Rating readonly :stars="5" v-model="data.stars" :cancel="false"></Rating>
                                    </div>
                                    ...
                                    <div class="flex flex-column md:flex-row flex-wrap md:justify-content-between mt-auto">
                                        <div class="flex gap-3">
                                            <p class="text-base flex align-items-center text-900 mt-0 mb-1">
                                                <i class="pi pi-users mr-2"></i>
                                                <span class="font-medium">{{ data.guest }}</span>
                                            </p>
                                            <p class="text-base flex align-items-center text-900 mt-0 mb-1">
                                                <i class="pi pi-wifi mr-2"></i>
                                                <span class="font-medium">{{ data.speed }}</span>
                                            </p>
                                            <p class="text-base flex align-items-center text-900 mt-0 mb-1">
...
<section class="surface-ground">
    <nav class="surface-overlay px-6 lg:px-3 xl:px-6 border-bottom-1 surface-border shadow-2 flex justify-content-between relative lg:static" style="min-height: 84px">
        <img src="/images/blocks/logos/bastion-700.svg" alt="Image" height="40" class="mr-0 lg:mr-6 align-self-center" />
        <a v-ripple class="p-ripple cursor-pointer block lg:hidden align-self-center text-700" v-styleclass="{ selector: '@next', enterClass: 'hidden', leaveToClass: 'hidden', hideOnOutsideClick: true }">
            <i class="pi pi-bars text-4xl"></i>
        </a>
        <div class="flex-grow-1 justify-content-between hidden lg:flex absolute lg:static w-full surface-overlay left-0 top-100 z-1 shadow-2 lg:shadow-none">
            <ul class="list-none p-0 m-0 flex select-none flex-column lg:flex-row">
                <li>
                    <a v-ripple class="p-ripple flex px-6 p-3 h-full lg:px-3 lg:py-2 align-items-center text-600 border-left-2 lg:border-bottom-2 lg:border-left-none border-transparent hover:border-blue-500 font-medium cursor-pointer transition-colors transition-duration-150">
                        <i class="pi pi-home mr-2"></i>
                        <span>Home</span>
                    </a>
                </li>
    ...
    <section class="surface-section px-6 py-5">
        ...
        <DataView ref="dv" :value="products" paginator :rows="10" layout="list" :rowsPerPageOptions="[10,25,50]" currentPageReportTemplate="Showing {first} to {last} of {totalRecords} entries" paginatorTemplate="CurrentPageReport FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown">
            <template #list="{data}">
                <article class="flex flex-column md:flex-row w-full gap-3 p-3 border-round border-1 surface-border mb-3">
                    <div class="relative">
                        <img :src="data.image" alt="Image" class="border-round w-full h-full md:w-14rem md:h-14rem" />
                    ...
                    <div class="flex flex-column w-full gap-3">
                        <div class="flex w-full justify-content-between align-items-center">
                            <p class="font-semibold text-lg mt-0 mb-0 mr-3">{{data.title}}</p>
                            <Rating readonly :stars="5" v-model="data.stars" :cancel="false"></Rating>
                        </div>
                        <p class="font-normal text-lg text-600 mt-0 mb-0">{{data.description}}</p>
                        <div class="flex flex-column md:flex-row flex-wrap md:justify-content-between mt-auto">
                            <div class="flex gap-3">
                                <p class="text-base flex align-items-center text-900 mt-0 mb-1">
                                    <i class="pi pi-users mr-2"></i>
                            ...
                            <div class="flex align-items-center">
                                <p class="font-semibold text-4xl text-900 mt-0 mb-0">{{data.price}}</p>
                                <i class="pi pi-circle-fill text-xs text-700 mx-2"></i>
                                <p class="flex align-items-center w-full text-lg text-700">9 nights</p>
                            </div>
                        </div>
...
`,
            ...
            rangeDates: [new Date('2022-11-16T11:52:51.542Z'), new Date('2023-01-16T11:52:51.542Z')],
            cities: [
            ...
            prices: [{ value: '$5000 - $10000' }, { value: '$10000 - $15000' }, { value: '$15000 - $20000' }],
            selectedPrice: { value: '$5000 - $10000' },
            selectedCities: [],
            products: [
                {
                    title: 'Private room in Amsterdam',
                    image: '/images/blocks/hotels/h4.jpeg',
                ...
                },
                {
                    ...
                    image: '/images/blocks/hotels/h5.jpeg',
                ...
                },
                {
                    ...
                    image: '/images/blocks/hotels/h6.jpeg',
